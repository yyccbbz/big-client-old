...
(function ($) {
    $.fn.extendPagination = function (options) {
        ...
        if (defaults.limit == '') {
            defaults.limit = '10';
        } else if (Number(defaults.limit) <= 0)defaults.limit = '10';
        ...
        if (totalPage > 0) {
            ...
            html.push('<li style="float:left;padding-top:3px;margin-right:7px">共'+totalCount+'条</li>');
            html.push('<li class="first"><a href="javascript:void(0);">|&lt;</a></li>');
            html.push('<li class="previous"><a href="javascript:void(0);">&lt;</a></li>');
            html.push('<li class="disabled hidden"><a href="javascript:void(0);">...</a></li>');
            if (totalPage <= showPage) {
                for (var i = 1; i <= totalPage; i++) {
                    if (i == 1) html.push('<li class="active"><a href="javascript:void(0);">' + i + '</a></li>');
                    else html.push('<li><a href="javascript:void(0);">' + i + '</a></li>');
                }
            } else {
                for (var j = 1; j <= showPage; j++) {
                    if (j == 1) html.push('<li class="active"><a href="javascript:void(0);">' + j + '</a></li>');
                    else html.push('<li><a href="javascript:void(0);">' + j + '</a></li>');
                }
            }
            html.push('<li class="disabled hidden"><a href="javascript:void(0);">...</a></li>');
            html.push('<li class="next"><a href="javascript:void(0);">&gt;</a></li>');
            html.push('<li class="last"><a href="javascript:void(0);">&gt;|</a></li></ul>');
            $(this).html(html.join(''));
            if (totalPage > showPage) $(this).find('ul.pagination li.next').prev().removeClass('hidden');

            var pageObj = $(this).find('ul.pagination'), firstObj = pageObj.find('li.first'), preObj = pageObj.find('li.previous'),
                currentObj = pageObj.find('li').not('.first,.previous,.disabled,.next,.last'),
                nextObj = pageObj.find('li.next'), lastObj = pageObj.find('li.last');
            ...
            firstObj.click(function (event) {
                ...
                if (totalPage > showPage) {
                    var minPage = 1;                  
                    var maxPage = minPage + showPage - 1;
                    loopPageElement(minPage, maxPage);
                }
                $.each(currentObj, function (index, thiz) {
                    if ($(thiz).find('a').html() == 1) {
                        activeObj.removeClass('active');
                        $(thiz).addClass('active');
                        callBack(1);
                    }
            ...
            lastObj.click(function (event) {
                ...
                if (totalPage > showPage) {
                    ...
                    var minPage = maxPage - showPage + 1;
                    loopPageElement(minPage, maxPage);             
                }
                $.each(currentObj, function (index, thiz) {
                    if ($(thiz).find('a').html() == totalPage) {
                        activeObj.removeClass('active');
                        $(thiz).addClass('active');
                        callBack(totalPage);
                    }
