...
$(document).ready(function(){
	...
	$("#group_org").change(change_group_name);
    $("#group_dep").keyup(change_group_name);
});

function change_group_name (){
	var group_org_val = $("#group_org").find("option:selected").text();
	var group_dep_val = $("#group_dep").val();
	if((group_org_val == null || group_org_val == "") && (group_dep_val == null || group_dep_val == "")){
		$("#groupname").val("");
	} else {
		$("#groupname").val(group_org_val +"_" + group_dep_val);
	}
}
