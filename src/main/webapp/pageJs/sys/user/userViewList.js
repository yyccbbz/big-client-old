...
function listData(pageDataObj) {
	...
	for (var i = 0; i < dataList.length; i++) {
		...
		trObj.append("<td class='tbody-td td_user_id td_padding_top'>" + user_id + "</td>");
		trObj.append("<td class='tbody-td td_user_name td_padding_top' title='" + user_nm + "'>" + user_nm + "</td>");
		trObj.append("<td class='tbody-td td_user_realname td_padding_top'>" + user_real_nm + "</td>");
		trObj.append("<td class='tbody-td td_user_creator td_padding_top' >" + create_name + "</td>");
		trObj.append("<td class='tbody-td td_user_date_and_time td_padding_top' >" + str_create_time + "</td>");
		trObj.append("<td class='tbody-td td_user_modifier td_padding_top'  >" + update_name + "</td>");
		trObj.append("<td class='tbody-td td_user_date_and_time td_padding_top' >" + str_update_time + "</td>");
		trObj.append("<td class='tbody-td td_user_operation td_padding_top'><a href = '"+webAppRoot+"/sys/user/userAddEdit.do?action=edit&currPage="+currPage+"&edit_user_id="+user_id+"&order_id="+order_id+"&order_type="+order_type+"'>编辑</a> | <a onclick='submitDel("+user_id+")'>删除</a></td>");
		/*if (status == 1) {
